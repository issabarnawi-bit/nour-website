...
begin
  ...
  and not exists (
    ...
    alter table public.roles rename column code to key;
  end if;
  ...
  and not exists (
    ...
    alter table public.roles rename column name to name_en;
  end if;
  ...
  and not exists (
    ...
    alter table public.roles rename column description to description_en;
  end if;
end;
$$;
...
update public.roles
set
  name_ar = coalesce(nullif(name_ar, ''), name_en, key),
...
begin
  ...
  and not exists (
    ...
    alter table public.permissions rename column code to key;
  end if;
  ...
  and not exists (
    ...
    alter table public.permissions rename column name to name_en;
  end if;
  ...
  and not exists (
    ...
    alter table public.permissions rename column description to description_en;
  end if;
end;
$$;
...
set
  module = coalesce(nullif(module, ''), split_part(key, '.', 1)),
  action = coalesce(nullif(action, ''), nullif(split_part(key, '.', 2), ''), 'access'),
  name_ar = coalesce(nullif(name_ar, ''), name_en, key),
  description_ar = coalesce(description_ar, description_en);

alter table public.permissions alter column module set not null;
alter table public.permissions alter column action set not null;
alter table public.permissions alter column name_ar set not null;
...
set search_path = public
as $$
  select public.has_role($1, 'super_admin');
$$;
...
set search_path = public
as $$
  select public.has_permission(auth.uid(), $1);
$$;
